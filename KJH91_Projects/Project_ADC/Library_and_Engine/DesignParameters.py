import re
#from generatorLib.generator_models.KJH91_Project.Library_and_Engine import user_define_exceptions
from KJH91_Projects.gds_editor_ver3 import user_define_exceptions
import sys
import os
_Technology='028nm'
_CadenceServer = '141.223.29.62'
...
if _Technology =='065nm':
    ...
    _UploadDirectory = '/mnt/sdb/kjh91/OPUS/CAD_S28nm_Workspace'
...
if _Technology == '028nm':
    _LayerMapping.update({'EG': _LayerMappingTmp[('EG', 'drawing')]})
...
print ('#########################   HDVNCAP Layer Mapping    #################################')
if _Technology == '028nm':
    _LayerMapping.update({'HDVNCAP_drawing': _LayerMappingTmp[('HDVNCAP', 'drawing')]})
    _LayerMapping.update({'HDVNCAP_parm': _LayerMappingTmp[('HDVNCAP', 'parm')]})
    _LayerMapping.update({'HDVNCAP_count': _LayerMappingTmp[('HDVNCAP', 'count')]})
    _LayerMapping.update({'M1_HDVNCAP': _LayerMappingTmp[('M1', 'HDVNCAP')]})
    _LayerMapping.update({'M2_HDVNCAP': _LayerMappingTmp[('M2', 'HDVNCAP')]})
    _LayerMapping.update({'M3_HDVNCAP': _LayerMappingTmp[('M3', 'HDVNCAP')]})
    _LayerMapping.update({'M4_HDVNCAP': _LayerMappingTmp[('M4', 'HDVNCAP')]})
    _LayerMapping.update({'M5_HDVNCAP': _LayerMappingTmp[('M5', 'HDVNCAP')]})
    _LayerMapping.update({'M6_HDVNCAP': _LayerMappingTmp[('M6', 'HDVNCAP')]})
    _LayerMapping.update({'M7_HDVNCAP': _LayerMappingTmp[('M7', 'HDVNCAP')]})

print ('#########################   DeepNwell Layer Mapping    #################################')
if _Technology == '028nm':
    _LayerMapping.update({'DEEPNWELL': _LayerMappingTmp[('T3', 'drawing')]})

print ('#########################   LVS1 Layer Mapping    #################################')
if _Technology == '028nm':
    _LayerMapping.update({'LVS1_dr8': _LayerMappingTmp[('LVS1', 'drawing8')]})
