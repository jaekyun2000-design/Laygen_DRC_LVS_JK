...
class _CommonArray(StickDiagram_KJH1._StickDiagram_KJH):
    ...
    def _CalculateDesignParameter(self,
        ...
        for i in range(_Bitsize) :
            ...
            self._DesignParameter['SRF_CDAC_B{}'.format(_Bitsize-i-1)] = self._SrefElementDeclaration(_DesignObj=H00_02_CapWithShield_KJH_test.
            _CapWithShield(_DesignParameter=None, _Name='{}:SRF_CDAC_B{}'.format(_Name, _Bitsize-i-1)))[0]
            ...
            for i in range(_Bitsize):
                ...
                ## Generate Sref: ex)self._DesignParameter['_NMOS_POWER'] = self._SrefElementDeclaration(_DesignObj=B16_nmos_power_v2._NMOS_POWER( _DesignParameter=None, _Name='{}:NMOS_POWER'.format(_Name)))[0]
                self._DesignParameter['SRF_CDAC_B{}'.format(_Bitsize - i - 1)] = \
                self._SrefElementDeclaration(_DesignObj=H00_02_CapWithShield_KJH_test.
                                             _CapWithShield(_DesignParameter=None,
                                                            _Name='{}:SRF_CDAC_B{}'.format(_Name, _Bitsize - i - 1)))[0]
                ...
                ## Calculate Sref Layer by using Calculation_Parameter: ex)'_NMOS_POWER'
                self._DesignParameter['SRF_CDAC_B{}'.format(_Bitsize - i - 1)]['_DesignObj']._CalculateDesignParameter(
...
if __name__ == '__main__':
    ...
    cellname = 'H00_00_CommonArray_v2'
    _fileName = cellname + '.gds'

    ''' Input Parameters for Layout Object ''' ############################################################### ^^^^^^^^^^^^^^^^^^^^^
    InputParams = dict(
        # # Element CDAC
        _LayoutOption=[2,3,4,5,6],
