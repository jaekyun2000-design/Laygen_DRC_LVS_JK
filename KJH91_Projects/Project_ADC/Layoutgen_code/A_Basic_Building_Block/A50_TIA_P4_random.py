...
## ########################################################################################################################################################## Class_HEADER
class _TIA_P4_YCH(StickDiagram_KJH1._StickDiagram_KJH):
    ...
    def _CalculateDesignParameter(self,
        ...
        _Caculation_Parameters['_PMOSNumberofGate'] = _Tr4_PMOSNumberofGate
        _Caculation_Parameters['_PMOSChannelWidth'] = _Tr4_PMOSChannelWidth
        _Caculation_Parameters['_PMOSChannellength'] = _Tr4_PMOSChannellength
        ...
        _Caculation_Parameters['_PMOSDummy'] = _Tr4_PMOSDummy
        _Caculation_Parameters['_PMOSDummy_length'] = _Tr4_PMOSDummy_length
        _Caculation_Parameters['_PMOSDummy_placement'] = _Tr4_PMOSDummy_placement
        ...
        self._DesignParameter['BND_Metal1Layer_Hrz_Gate_extension']['_YWidth'] = abs(
            tmp2[0][0][0][0]['_XY_down'][1] - tmp1[0][0]['_XY_up'][1])
        ...
        ## ################################################################################################################### Metal2_Layer Gate
        # Define Boundary_element
        self._DesignParameter['BND_Metal2Layer_Hrz_Gate'] = self._BoundaryElementDeclaration(
        ...
        # tmp2 = self.get_param_KJH4('SRF_Gate_ViaM0M1', 'SRF_ViaM0M1', 'BND_Met1Layer')
        self._DesignParameter['BND_Metal2Layer_Hrz_Gate']['_YWidth'] = tmp1[0][0]['_Ywidth']

        # Define Boundary_element _XWidth
        self._DesignParameter['BND_Metal2Layer_Hrz_Gate']['_XWidth'] = tmp1[0][0]['_Xwidth']

        # Calculate Sref XYcoord
        # initialize coordinate
        self._DesignParameter['BND_Metal2Layer_Hrz_Gate']['_XYCoordinates'] = [[0, 0]]
        tmpXY = []
        ...
        target_coord = tmp1[0][0]['_XY_cent']
        # Approaching_coord
        tmp2 = self.get_param_KJH4('BND_Metal2Layer_Hrz_Gate')
        approaching_coord = tmp2[0][0]['_XY_cent']
        # Sref coord
        tmp3 = self.get_param_KJH4('BND_Metal2Layer_Hrz_Gate')
        Scoord = tmp3[0][0]['_XY_origin']
        # Cal
        New_Scoord = self.get_Scoord_KJH4(target_coord, approaching_coord, Scoord)
        tmpXY.append(New_Scoord)
        # Define
        self._DesignParameter['BND_Metal2Layer_Hrz_Gate']['_XYCoordinates'] = tmpXY
        ...
        # Sref ViaX declaration
        self._DesignParameter['SRF_Gate_ViaM1M2'] = self._SrefElementDeclaration(
            _DesignObj=A02_ViaStack_KJH2._ViaStack_KJH2(_DesignParameter=None,
                                                        _Name='{}:SRF_Gate_ViaM1M2'.format(_Name)))[0]

        # Define Sref Relection
        self._DesignParameter['SRF_Gate_ViaM1M2']['_Reflect'] = [0, 0, 0]

        # Define Sref Angle
        self._DesignParameter['SRF_Gate_ViaM1M2']['_Angle'] = 0
        ...
        # Calculate _COX
        # Calculate Number of V1
        tmp = self.get_param_KJH4('BND_Metal2Layer_Hrz_Gate')
        M1_Xwidth = tmp[0][0]['_Xwidth']
        Num_V1 = int((M1_Xwidth - 2 * _DRCObj._Metal1MinEnclosureVia3) / (_DRCObj._VIAxMinWidth + _DRCObj._VIAxMinSpace))
        # Define Num of V1
        if Num_V1 < 2:
            _Caculation_Parameters['_COX'] = 2
        else:
        ...
        # Generate Metal(x), Metal(x+1) and C0(Viax) layer
        self._DesignParameter['SRF_Gate_ViaM1M2']['_DesignObj']._CalculateDesignParameterXmin(**_Caculation_Parameters)

        # Calculate Sref XYcoord
        tmpXY = []
        # initialized Sref coordinate
        self._DesignParameter['SRF_Gate_ViaM1M2']['_XYCoordinates'] = [[0, 0]]
        ...
        # Calculate
        # Target_coord
        tmp1 = self.get_param_KJH4('BND_Metal2Layer_Hrz_Gate')
        target_coord = tmp1[0][0]['_XY_cent']
        # Approaching_coord
        tmp2 = self.get_param_KJH4('SRF_Gate_ViaM1M2', 'SRF_ViaM1M2', 'BND_Met1Layer')
        approaching_coord = tmp2[0][0][0][0]['_XY_cent']
        # Sref coord
        tmp3 = self.get_param_KJH4('SRF_Gate_ViaM1M2')
        Scoord = tmp3[0][0]['_XY_origin']
        # Cal
        New_Scoord = self.get_Scoord_KJH4(target_coord, approaching_coord, Scoord)
        tmpXY.append(New_Scoord)

        self._DesignParameter['SRF_Gate_ViaM1M2']['_XYCoordinates'] = tmpXY
...
## ################################################################################################################### Metal3_Layer_Drain
        ...
        if (_Tr4_PMOSNumberofGate % 2 == 1):
            for i in range(0, int(_Tr4_PMOSNumberofGate / 2) + 1):
                ...
                target_coord[1] = target_coord[1] - tmp1[0][i][0]['_Ywidth']/4
                # Approaching_coord
            ...
            for i in range(0, int(_Tr4_PMOSNumberofGate / 2)):
                # Calculate
                # Target_coord
                tmp1 = self.get_param_KJH4('SRF_Pmos', 'BND_Met1Layer_Drain')
                target_coord = tmp1[0][i][0]['_XY_up_left']
...
## ################################################################################################################### Drain_ViaM2M3
        ...
        # Define Boundary_element
        self._DesignParameter['BND_Metal3Layer_Connect_P4_Drain'] = self._BoundaryElementDeclaration(
                                                                                                    _Layer=DesignParameters._LayerMapping['METAL3'][0],
                                                                                                    _Datatype=DesignParameters._LayerMapping['METAL3'][1],
                                                                                                    _XWidth=None,
                                                                                                    _YWidth=None,
        ...
        # Define Boundary_element _YWidth
        self._DesignParameter['BND_Metal3Layer_Connect_P4_Drain']['_YWidth'] = tmp1[0][0]['_Ywidth'] / 2

        # Define Boundary_element _XWidth
        self._DesignParameter['BND_Metal3Layer_Connect_P4_Drain']['_XWidth'] = abs(
            tmp1[-1][0]['_XY_right'][0] - tmp1[0][0]['_XY_left'][0])

        # Calculate Sref XYcoord
        # initialize coordinate
        self._DesignParameter['BND_Metal3Layer_Connect_P4_Drain']['_XYCoordinates'] = [[0, 0]]
        tmpXY = []
        ...
        target_coord = tmp1[0][0]['_XY_down_left']
        # Approaching_coord
        tmp2 = self.get_param_KJH4('BND_Metal3Layer_Connect_P4_Drain')
        approaching_coord = tmp2[0][0]['_XY_down_left']
        # Sref coord
        tmp3 = self.get_param_KJH4('BND_Metal3Layer_Connect_P4_Drain')
        Scoord = tmp3[0][0]['_XY_origin']
        # Cal
        New_Scoord = self.get_Scoord_KJH4(target_coord, approaching_coord, Scoord)
        tmpXY.append(New_Scoord)
        # Define
        self._DesignParameter['BND_Metal3Layer_Connect_P4_Drain']['_XYCoordinates'] = tmpXY
...
if __name__ == '__main__':
    ...
    LayoutObj = _TIA_P4_YCH(_DesignParameter=None, _Name=cellname)
