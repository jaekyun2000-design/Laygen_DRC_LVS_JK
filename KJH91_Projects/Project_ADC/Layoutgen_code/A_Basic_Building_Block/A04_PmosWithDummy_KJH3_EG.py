...
## ########################################################################################################################################################## Class_HEADER
class _PmosWithDummy(StickDiagram_KJH1._StickDiagram_KJH):
...
_PMOSXvt_Minexten=True,  # True/False
# XVT setting : None(Cent), Up, Dn
_PMOSXvt_placement=None,  # None/'Up'/'Dn'/
...
_PMOSXvt_placement=None,  # None/'Up'/'Dn'/
        ...
        self._DesignParameter['BND_POLayer'] = self._BoundaryElementDeclaration(
                                                                            _Layer=
                                                                            DesignParameters._LayerMapping['POLY'][0],
                                                                            _Datatype=
                                                                            DesignParameters._LayerMapping['POLY'][1],
                                                                            _XWidth=None,
                                                                            _YWidth=None,
                                                                            _XYCoordinates=[ ],
                                                                           )
        ...
        self._DesignParameter['BND_POLayer']['_YWidth'] = _PMOSChannelWidth + 2 * _DRCObj.DRCPolygateMinExtensionOnOD(_PMOSChannellength)

        #Define Boundary_element _XWidth
        self._DesignParameter['BND_POLayer']['_XWidth'] = _PMOSChannellength
        ...
        if _PMOSDummy == False:
            ...
            self._DesignParameter['BND_PODummyLayer']['_YWidth'] = _PMOSChannelWidth + 2 * a

        #Define Boundary_element _XWidth
        if _PMOSDummy == False:
            self._DesignParameter['BND_PODummyLayer']['_XWidth'] = 0
        elif _PMOSDummy:
            self._DesignParameter['BND_PODummyLayer']['_XWidth'] = _PMOSChannellength
        ...
        for i in [0,-1]:
            if i == 0:
                ...
                Scoord = tmp3[0][0]['_XY_origin']
                        #Cal
                New_Scoord = self.get_Scoord_KJH4(target_coord,approaching_coord,Scoord)
                New_Scoord[0] = New_Scoord[0] - _DRCObj._PODummyMinSpace2 - _PMOSChannellength
                tmpXY.append(New_Scoord)
            else:
                    #Calculate
                        #Target_coord
        ...
        #Define Boundary_element _YWidth
        self._DesignParameter['BND_ODLayer']['_YWidth'] = _PMOSChannelWidth
        ...
        self._DesignParameter['BND_Met1Layer'] = self._BoundaryElementDeclaration(
                                                                                _Layer=
                                                                                DesignParameters._LayerMapping['METAL1'][0],
                                                                                _Datatype=
                                                                                DesignParameters._LayerMapping['METAL1'][1],
                                                                                _XWidth=None,
                                                                                _YWidth=None,
                                                                                _XYCoordinates=[ ],
                                                                               )
        ...
        _XYCoordinateOfPMOS = [New_Scoord]
        ...
        try:
            if (DesignParameters._Technology == '028nm') and _XVT in ('SLVT', 'LVT', 'RVT', 'HVT', 'EG'):
            ...
            if _XVTLayer != None:
                ...
                if _PMOSXvt_Minexten == True:
                    area = (self._DesignParameter['BND_ODLayer']['_YWidth'] + 2 * _DRCObj._XvtMinEnclosureOfODY) * (
                                self._DesignParameter['BND_ODLayer']['_XWidth'] + 2 * _DRCObj._XvtMinEnclosureOfODX)

                    if area > 160000:
                        # Define Boundary_element _YWidth
                        self._DesignParameter[_XVTLayer]['_YWidth'] = self._DesignParameter['BND_ODLayer'][
                                                                          '_YWidth'] + 2 * _DRCObj._XvtMinEnclosureOfODY
                        # Define Boundary_element _XWidth
                        self._DesignParameter[_XVTLayer]['_XWidth'] = self._DesignParameter['BND_ODLayer'][
                                                                          '_XWidth'] + 2 * _DRCObj._XvtMinEnclosureOfODX
                    else:
                        # Define Boundary_element _XWidth
                        self._DesignParameter[_XVTLayer]['_XWidth'] = self._DesignParameter['BND_ODLayer'][
                                                                          '_XWidth'] + 2 * _DRCObj._XvtMinEnclosureOfODX
                        # Define Boundary_element _YWidth
                        self._DesignParameter[_XVTLayer]['_YWidth'] = np.ceil(
                            160000 / np.array(self._DesignParameter[_XVTLayer]['_XWidth']))
        ...
        if DesignParameters._Technology == '028nm' and _PCCrit != False:
            ...
            if self._DesignParameter['BND_POLayer']['_XWidth'] in (30, 34):
                ...
                #Define Boundary_element _XWidth
                self._DesignParameter['BND_PCCRITLayer']['_XWidth'] = _LengthPMOSBtwMet1 * _PMOSNumberofGate + _DRCObj._CoMinWidth + 2 * _DRCObj._CoMinEnclosureByOD + 2 * _DRCObj._PCCRITExtension
       ...
       #Define Boundary_element
        ...
        ## Define Boundary_element _YWidth
        tmp0 = self.get_param_KJH4('BND_PPLayer')
        self._DesignParameter['BND_PMOS_NWell']['_YWidth'] = abs(tmp0[0][0]['_XY_up'][1] - tmp0[0][0]['_XY_down'][1]) + 2*_DRCObj._NwMinEnclosurePactive2

        ## Define Boundary_element _XWidth
        self._DesignParameter['BND_PMOS_NWell']['_XWidth'] = abs(tmp0[0][0]['_XY_right'][0] - tmp0[0][0]['_XY_left'][0]) + 2*_DRCObj._NwMinEnclosurePactive2
        ...
        tmp1 = self.get_param_KJH4('BND_PPLayer')
        target_coord = tmp1[0][0]['_XY_cent']
        ...
        if _PMOSDummy == True:
            ...
            if _PMOSDummy_length == None:
                ...
                self._DesignParameter['BND_PODummyLayer']['_YWidth'] = _PMOSDummy_length

                ## ##################################################################################################### POLY_Dummy_Layer2:manual setting:dummy_placement
            if _PMOSDummy_placement == None:

                # Poly minimum area constraint: place to 'Dn'
                if flag == 1:
                    # Calculate Sref XYcoord
                    # initialized Sref coordinate
                    self._DesignParameter['BND_PODummyLayer']['_XYCoordinates'] = [[0, 0]]
...
if __name__ == '__main__':
    ...
    libname = 'Proj_ADC_A_building_block_PMOSwithdummy_inverter_test_v1'
    cellname = 'A04_PmosWithDummy_KJH3_88'
...
_PMOSXvt_placement=None,  # None/'Up'/'Dn'/
    ...
    LayoutObj = _PmosWithDummy(_DesignParameter=None, _Name=cellname)
