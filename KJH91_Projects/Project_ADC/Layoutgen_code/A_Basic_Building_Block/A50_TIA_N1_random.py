...
## ########################################################################################################################################################## Class_HEADER
class _TIA_N1_YCH(StickDiagram_KJH1._StickDiagram_KJH):
    ...
    def _CalculateDesignParameter(self,
        ...
        if (_Tr1_NMOSNumberofGate % 2 == 1):
            ...
            for i in range(0, int(_Tr1_NMOSNumberofGate / 2)):
                # Calculate
                # Target_coord
                tmp1 = self.get_param_KJH4('SRF_Nmos', 'BND_Met1Layer_Drain')
                target_coord = tmp1[0][i][0]['_XY_up_left']
        ...
        # Define Boundary_element
        self._DesignParameter['BND_Metal3Layer_Connect_N1_Drain'] = self._BoundaryElementDeclaration(
                                                                                                _Layer=DesignParameters._LayerMapping['METAL3'][0],
                                                                                                _Datatype=DesignParameters._LayerMapping['METAL3'][1],
                                                                                                _XWidth=None,
                                                                                                _YWidth=None,
        ...
        # Define Boundary_element _YWidth
        self._DesignParameter['BND_Metal3Layer_Connect_N1_Drain']['_YWidth'] = tmp1[0][0]['_Ywidth'] / 2

        # Define Boundary_element _XWidth
        self._DesignParameter['BND_Metal3Layer_Connect_N1_Drain']['_XWidth'] = abs(
            tmp1[-1][0]['_XY_right'][0] - tmp1[0][0]['_XY_left'][0])

        # Calculate Sref XYcoord
        # initialize coordinate
        self._DesignParameter['BND_Metal3Layer_Connect_N1_Drain']['_XYCoordinates'] = [[0, 0]]
        tmpXY = []
        ...
        # Approaching_coord
        tmp2 = self.get_param_KJH4('BND_Metal3Layer_Connect_N1_Drain')
        approaching_coord = tmp2[0][0]['_XY_down_left']
        # Sref coord
        tmp3 = self.get_param_KJH4('BND_Metal3Layer_Connect_N1_Drain')
        Scoord = tmp3[0][0]['_XY_origin']
        # Cal
        New_Scoord = self.get_Scoord_KJH4(target_coord, approaching_coord, Scoord)
        tmpXY.append(New_Scoord)
        # Define
        self._DesignParameter['BND_Metal3Layer_Connect_N1_Drain']['_XYCoordinates'] = tmpXY
...
if __name__ == '__main__':
    ...
    LayoutObj = _TIA_N1_YCH(_DesignParameter=None, _Name=cellname)
