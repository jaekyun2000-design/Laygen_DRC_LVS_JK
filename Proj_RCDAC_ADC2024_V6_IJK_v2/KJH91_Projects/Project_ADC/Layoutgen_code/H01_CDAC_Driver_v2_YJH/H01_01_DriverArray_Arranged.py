...
from Proj_RCDAC_ADC2024_V6_IJK_v2.KJH91_Projects.Project_ADC.Layoutgen_code.H01_CDAC_Driver_v2_YJH import H01_00_DriverCell
...
from Proj_RCDAC_ADC2024_V6_IJK_v2.KJH91_Projects.Project_ADC.Layoutgen_code.A_Basic_Building_Block import A07_NbodyContactPhyLen_KJH2
...
from Proj_RCDAC_ADC2024_V6_IJK_v2.KJH91_Projects.Project_ADC.Layoutgen_code.A_Basic_Building_Block import A08_PbodyContactPhyLen_KJH2
...
############################################################################################################################################################ Class_HEADER
class _DriverArray_Arranged(StickDiagram_KJH1._StickDiagram_KJH):
    ##########################################################################################################################
    ...
    def _CalculateDesignParameter(self,
        ...
        for i in range(_NumOfBits):
            ...
            _Caculation_Parameters0['_Driver_NMOS_NumberofGate'] = _Driver_NMOS_NumberofGate * _Driver_SizeByBit[i]
            _Caculation_Parameters0['_Driver_PMOS_NumberofGate'] = _Driver_PMOS_NumberofGate * _Driver_SizeByBit[i]
            ...
            tmpDrvXY.append([tmpDrvXY[i][0] + DistanceBtwTr,0])
            # Define coord
            self._DesignParameter['SRF_{}C_DriverCell'.format(CapSize)]['_XYCoordinates'] = [tmpDrvXY[i]]
        ...
        # 마지막 index 'tmpDrvXY' 그대로 사용!!!!!!
        print('#########################################')
        print('##    Dummy Driver Calculation_Start    ##')
        print('#########################################')

        _Caculation_Parameters0['_Driver_NMOS_NumberofGate'] = 1
        _Caculation_Parameters0['_Driver_PMOS_NumberofGate'] = 1

        self._DesignParameter['SRF_Dummy_DriverCell'] = self._SrefElementDeclaration(
            _DesignObj=H01_00_DriverCell._DriverCell(_DesignParameter=None, _Name='{}:SRF_Dummy_DriverCell'.format(_Name)))[0]
        ...
        ## Calculate Sref Layer by using Calculation_Parameter: ex)'_NMOS_POWER'
        self._DesignParameter['SRF_Dummy_DriverCell']['_DesignObj']._CalculateDesignParameter(**_Caculation_Parameters0)

        ## initialize Sref _XYcoordinate: ex)'_NMOS_POWER'
        self._DesignParameter['SRF_Dummy_DriverCell']['_XYCoordinates'] = [[0, 0]]

        tmp1 = self.get_param_KJH4('SRF_Dummy_DriverCell', 'SRF_Driver_NMOS', 'BND_PODummyLayer')
        tmp2 = self.get_param_KJH4('SRF_Dummy_DriverCell', 'SRF_Driver_PMOS', 'BND_PODummyLayer')
        ...
        if NMOSXwidth > PMOSXwidth:
            ...
            DistanceBtwTr = PMOSXwidth + 96
        tmpDrvXY.append([tmpDrvXY[_NumOfBits][0] + DistanceBtwTr,0])
        # Define coord
        self._DesignParameter['SRF_Dummy_DriverCell']['_XYCoordinates'] = [tmpDrvXY[_NumOfBits]]
        ...
        tmp = self.get_param_KJH4('SRF_{}C_DriverCell'.format(MaxCapSize), 'SRF_Driver_PMOS', 'BND_PODummyLayer')
        tmp2 = self.get_param_KJH4('SRF_Dummy_DriverCell', 'SRF_Driver_PMOS', 'BND_PODummyLayer')

        _Caculation_Parameters['_Length'] = max(abs(tmp[0][0][0][0]['_XY_left'][0] - tmp2[0][0][-1][0]['_XY_right'][0]), 3300)
        ...
        target_coord = [int((tmp[0][0][0][0]['_XY_left'][0] + tmp2[0][0][-1][0]['_XY_right'][0])/2), tmp1[0][0][0][0]['_XY_up'][1]]
        ...
        tmp = self.get_param_KJH4('SRF_{}C_DriverCell'.format(MaxCapSize), 'SRF_Driver_NMOS', 'BND_PODummyLayer')
        tmp2 = self.get_param_KJH4('SRF_Dummy_DriverCell', 'SRF_Driver_NMOS', 'BND_PODummyLayer')
        _Caculation_Parameters['_Length'] = max(abs(tmp[0][0][0][0]['_XY_left'][0] - tmp2[0][0][-1][0]['_XY_right'][0]), 3300)
        _Caculation_Parameters['_NumCont'] = 2
        _Caculation_Parameters['_Vtc_flag'] = False
        ...
        ## Calculate
        ## Target_coord: _XY_type1
        tmp1 = self.get_param_KJH4('SRF_Dummy_DriverCell', 'SRF_Driver_NMOS', 'BND_{}Layer'.format(_Driver_NMOS_XVT))
        target_coord = [int((tmp[0][0][0][0]['_XY_left'][0] + tmp2[0][0][-1][0]['_XY_right'][0])/2), tmp1[0][0][0][0]['_XY_down'][1]]
        ...
        tmp0 = self.get_param_KJH4('SRF_{}C_DriverCell'.format(MaxCapSize), 'SRF_Driver_PMOS', 'BND_{}Layer'.format(_Driver_PMOS_XVT))
        ...
        ## Define Boundary_element _YWidth
        tmp0 = self.get_param_KJH4('SRF_{}C_DriverCell'.format(MaxCapSize), 'SRF_Driver_PMOS', 'BND_PPLayer')
        ...
        ## Define Boundary_element _YWidth
        tmp0 = self.get_param_KJH4('SRF_{}C_DriverCell'.format(MaxCapSize), 'SRF_Driver_NMOS', 'BND_{}Layer'.format(_Driver_NMOS_XVT))
        tmp1 = self.get_param_KJH4('SRF_Dummy_DriverCell', 'SRF_Driver_NMOS', 'BND_{}Layer'.format(_Driver_NMOS_XVT))
        ...
        tmp = self.get_param_KJH4('SRF_{}C_DriverCell'.format(MaxCapSize), 'SRF_NMOSSourceVREFN_ViaM2M6', 'SRF_ViaM5M6','BND_Met6Layer')
        ...
        ## Define Boundary_element _YWidth
        VREFN_Hrz_M6_Minwidth = 244 # 비아당 컨택 두 개 박힐려면 최소 244 Ywidth가 필요함.
        tmp = self.get_param_KJH4('SRF_{}C_DriverCell'.format(MaxCapSize), 'SRF_PMOSSourceVREFP_ViaM2M6', 'SRF_ViaM5M6','BND_Met6Layer')
...
if __name__ == '__main__':
    ...
    libname = 'Proj_RcdacSar_H01_CDACDriver_Arr'
    ## CellName: ex)C01_cap_array_v2_84
    cellname = 'H01_01_DriverArray_Arr_3bit'
    ...
    InputParams = dict(
        # NumOfDriverUnitsInDriverArray = 2^(_NumOfBits) - 1
        _NumOfBits=8,
        _Driver_SizeByBit=[128,64,32,16,8,4,2,1],
    ...
    ## Gen Object:
    LayoutObj = _DriverArray_Arranged(_DesignParameter=None, _Name=cellname)
