    ...
    def _CalculateDesignParameter(self,
        ...
        if _GateSpacing != None:
            if (_GateSpacing + _ChannelLength) < _LengthBtwPO:
                ...
                _LengthBtwPO = _GateSpacing + _ChannelLength
        ...
        for i in range(0,_NumberofGate-1):
            ...
            target_coord = tmp1[i][0]['_XY_down_left']
                    #Approaching_coord
            # tmp2 = self.get_param_KJH4('BND_POLayer') #to sped up
            approaching_coord = tmp2[0][0]['_XY_down_left']
                    #Sref coord
            # tmp3 = self.get_param_KJH4('BND_POLayer') # to sped up
        ...
        if _PODummy_TF == False:
            ...
            self._DesignParameter['BND_PODummyLayer']['_YWidth'] = self._DesignParameter['BND_POLayer']['_YWidth']  ### 231112
        ...
        for i in [0,-1]:
            if i == 0:
                ...
                tmp1 = self.get_param_KJH4('BND_POLayer')
                tmp2 = self.get_param_KJH4('BND_PODummyLayer')
                Ydel = np.ceil( 0.5 * abs(tmp2[0][0]['_Ywidth'] - tmp1[0][0]['_Ywidth']))
                if tmp1[0][0]['_Ywidth']>tmp2[0][0]['_Ywidth']:
                    New_Scoord[1] = New_Scoord[1] + Ydel
                else:
                    New_Scoord[1] = New_Scoord[1] - Ydel
                tmpXY.append(New_Scoord)
            else:
        ...
        tmp1 = self.get_param_KJH4('BND_POLayer')
        tmp2 = self.get_param_KJH4('BND_ODLayer')
        Xdel = 0.5 * abs( abs(tmp1[-1][0]['_XY_right'][0]-tmp1[0][0]['_XY_left'][0]) - tmp2[0][0]['_Xwidth'] )
        New_Scoord[0] = New_Scoord[0] - Xdel
                # New_ScoordY
        tmp1 = self.get_param_KJH4('BND_POLayer')
        tmp2 = self.get_param_KJH4('BND_ODLayer')
        Ydel = 0.5 * abs( tmp1[0][0]['_Ywidth']- tmp2[0][0]['_Ywidth'])
        New_Scoord[1] = New_Scoord[1] + Ydel
        ...
        New_Scoord = self.get_Scoord_KJH4(target_coord,approaching_coord,Scoord)
                #New_ScoordX
        New_Scoord[0] = New_Scoord[0] + 3
                #New_ScoordY
        tmp1 = self.get_param_KJH4('BND_ODLayer')
        tmp2 = self.get_param_KJH4('BND_Met1Layer')
        if tmp1[0][0]['_Ywidth'] > tmp2[0][0]['_Ywidth']:
            New_Scoord[1] = New_Scoord[1] - np.ceil(0.5 * abs(tmp1[0][0]['_Ywidth'] - tmp2[0][0]['_Ywidth']))
        else:
            New_Scoord[1] = New_Scoord[1] + np.ceil(0.5 * abs(tmp1[0][0]['_Ywidth'] - tmp2[0][0]['_Ywidth']))
        ...
        tmpXYs = []
        for i in range(0, _NumberofGate + 1):
            XY = [_XYCoordinateOfNMOS[0][0] + i * _LengthBtwMet1,_XYCoordinateOfNMOS[0][1]]
            tmpXYs.append(XY)
            #Define coord
        ...
        New_Scoord = self.get_Scoord_KJH4(target_coord, approaching_coord, Scoord)
            #New_scoordX
        New_Scoord[0] = New_Scoord[0] + 5
            #New_scoordY
        New_Scoord[1] = New_Scoord[1] + 21
        ...
        for i in range(0, _XNumberOfCOInNMOS):
            for j in range(0, _YNumberOfCOInNMOS):
                XY = [_XYCoordinateOfNMOS[0][0] + i * _LengthBtwMet1,_XYCoordinateOfNMOS[0][1] + j * _LengthBtwCO]
                tmpXYs.append(XY)
        ...
        try:
            ...
            if _XVTLayer != None:
                ...
                if _Xvt_Placement == None:
                    if area > 160000:
                        New_Scoord[0] = New_Scoord[0] - _DRCObj._XvtMinEnclosureOfODX
                        New_Scoord[1] = New_Scoord[1] - _DRCObj._XvtMinEnclosureOfODY
                    else:
                        New_Scoord[0] = New_Scoord[0] - _DRCObj._XvtMinEnclosureOfODX
                        New_Scoord[1] = New_Scoord[1] - np.ceil((np.ceil(160000 / np.array(self._DesignParameter[_XVTLayer]['_XWidth'])))/2)
                elif _Xvt_Placement == 'Up':
                    if area > 160000:
                        New_Scoord[0] = New_Scoord[0] - _DRCObj._XvtMinEnclosureOfODX
                        New_Scoord[1] = New_Scoord[1] - _DRCObj._XvtMinEnclosureOfODY
        ...
        if _MosType == 'PMOS':
            ...
            New_Scoord[0] = New_Scoord[0] - _DRCObj._PpMinEnclosureOfPactiveX
            New_Scoord[1] = New_Scoord[1] - _DRCObj._PpMinEnclosureOfPactiveY
        ...
        if DesignParameters._Technology == '028nm' and _PCCrit != False:
            ...
            if self._DesignParameter['BND_POLayer']['_XWidth'] in (30, 34):
                ...
                self._DesignParameter['BND_PCCRITLayer']['_XWidth'] = abs( tmp[-1][0]['_XY_right'][0] - tmp[0][0]['_XY_left'][0] ) + 2 * _DRCObj._PCCRITExtension
                ...
                target_coordx = tmp1_1[0][0]['_XY_left'][0]
                    #Y
                tmp1_2 = self.get_param_KJH4('BND_ODLayer')
                ...
                # Cal
                New_Scoord = self.get_Scoord_KJH4(target_coord, approaching_coord, Scoord)
                New_Scoord[0] = New_Scoord[0] -  _DRCObj._PCCRITExtension
                New_Scoord[1] = New_Scoord[1] - _DRCObj._PCCRITExtension
        ...
        #Define Boundary_element _YWidth (always even)
        tmp1 = self.get_param_KJH4('BND_POLayer')
        tmp2 = self.get_param_KJH4('BND_ODLayer')
        self._DesignParameter['BND_POLayerPINDrawing']['_YWidth'] = abs(tmp1[0][0]['_XY_up'][1] - tmp2[0][0]['_XY_up'][1])
        ...
        for i in range(0,len(tmp)):
            ...
            # Approaching_coord
            # tmp2 = self.get_param_KJH4('BND_POLayerPINDrawing') # to sped up
            approaching_coord = tmp2[0][0]['_XY_up_left']
            # Sref coord
            # tmp3 = self.get_param_KJH4('BND_POLayerPINDrawing') # to sped up
            Scoord = tmp3[0][0]['_XY_origin']
            # Cal
            New_Scoord = self.get_Scoord_KJH4(target_coord, approaching_coord, Scoord)
            tmpxy.append(New_Scoord)
            ...
            if _POGate_Via_TF == True:
                ...
                if flag == 0:
                    ...
                    if _MosUpDn == 'Up':
                        ...
                        tmp2 = self.get_param_KJH4('SRF_Gate_ViaM0Mx', 'SRF_ViaM{}M{}'.format(_POGate_ViaMxMx[0],_POGate_ViaMxMx[0]+1),'BND_POLayer')
        ...
        ## ########## ########## ########## ########## ########## ########## ########## ########## ########## ########## POLY_Dummy_Layer2:manual setting:dummy_length
            if _PODummy_Length == None:
                ...
                # Check Poly minimum area constraint
...
if __name__ == '__main__':
    ...
    for _iter in range(5):
        cellname = 'A14_Mosfet_v{}'.format(_iter+1)
...
#Mosfet
        ...
        # Physical dimension
        _NumberofGate	            = random.randrange(1, 10, 1),       # Number
        _ChannelWidth	            = random.randrange(1000, 10000, 1000), #625,      # Number
        _ChannelLength	            = 30,       # Number
        _GateSpacing		        = None,     # None/Number
        _SDWidth			        = None,     # None/Number
        _XVT				        = 'SLVT',   # 'XVT' ex)SLVT/LVT/RVT/HVT
        ...
        _Xvt_MinExten_TF            = True,     # True/False
        # XVT setting : None(Cent), Up, Dn
        _Xvt_Placement              = None,     # None/'Up'/'Dn'/
        ...
        # Poly Gate Via setting
        _POGate_Via_TF              = True,     # True/False
            # Poly Gate Via setting :
        _POGate_ViaMxMx             = [0,1]     # Ex) [1,5] -> ViaM1M5
