...
if __name__ == '__main__':
    ...
    for _iter in range(5):
        ...
        cellname = 'C10_00_VddTieCell8_v{}'.format(_iter + 1)
        _fileName = cellname + '.gds'

        _Tie8_Gate = random.randrange(1, 5, 1)
        _Tie8_Width = random.randrange(400, 1000, 100)

        ''' Input Parameters for Layout Object ''' ############################################################### ^^^^^^^^^^^^^^^^^^^^^
        InputParams = dict(
    # Tie8N
        # Physical dimension
        _Tie8N_NumberofGate	            = _Tie8_Gate,       # Number
        _Tie8N_ChannelWidth	            = _Tie8_Width,     # Number
        _Tie8N_ChannelLength	        = 30,       # Number
        _Tie8N_XVT				        = 'SLVT',    # 'XVT' ex)SLVT/LVT/RVT/HVT

    # Tie8P
        # Physical dimension
        _Tie8P_NumberofGate	            = _Tie8_Gate,       # Number
        _Tie8P_ChannelWidth	            = _Tie8_Width,     # Number
        _Tie8P_ChannelLength	        = 30,       # Number
        _Tie8P_XVT				        = 'SLVT',    # 'XVT' ex)SLVT/LVT/RVT/HVT
        )
