...
class _Inverter(StickDiagram._StickDiagram):
    ...
    def _CalculateDesignParameter_v3(self,
        ...
        self._SrefElementDeclaration(_DesignObj=NMOSWithDummy._NMOS(_Name='NMOS_In{}'.format(_Name)))[0]
        ...
        self._SrefElementDeclaration(_DesignObj=PMOSWithDummy._PMOS(_Name='PMOS_In{}'.format(_Name)))[0]
        ...
        if '_PODummyLayer' in self._DesignParameter['_NMOS']['_DesignObj']._DesignParameter:
            ...
            if Area_NmosDummy < _DRCObj._PODummyMinArea:
                YWidth_NmosDummy_Recalc = self.CeilMinSnapSpacing(
                    _DRCObj._PODummyMinArea / self.getXWidth('_NMOS', '_PODummyLayer'), MinSnapSpacing * 2)
                self._DesignParameter['_NMOS']['_DesignObj']._DesignParameter['_PODummyLayer'][
                    '_YWidth'] = YWidth_NmosDummy_Recalc
            else:
                pass
        else:
            pass
        if '_PODummyLayer' in self._DesignParameter['_PMOS']['_DesignObj']._DesignParameter:
            ...
            if Area_PmosDummy < _DRCObj._PODummyMinArea:
                YWidth_PmosDummy_Recalc = self.CeilMinSnapSpacing(
                    _DRCObj._PODummyMinArea / self.getXWidth('_PMOS', '_PODummyLayer'), MinSnapSpacing * 2)
                self._DesignParameter['_PMOS']['_DesignObj']._DesignParameter['_PODummyLayer'][
                    '_YWidth'] = YWidth_PmosDummy_Recalc
            else:
                pass
        else:
            pass
        ...
        if _SupplyRailType == 1:
            ...
            self._DesignParameter['ViaForVDD'] = \
            self._SrefElementDeclaration(_DesignObj=Z_PWR_CNT.Z_PWR_CNT(_Name='ViaForVDDIn{}'.format(_Name)))[0]
            self._DesignParameter['ViaForVSS'] = \
            self._SrefElementDeclaration(_DesignObj=Z_PWR_CNT.Z_PWR_CNT(_Name='ViaForVSSIn{}'.format(_Name)))[0]
        ...
        if _NumViaPMOSMet12Met2CoY == None:  # Default : calculate
            ...
            NumViaYPMOS = int((YWidthOfPMOSMet1 - 2 * _DRCObj._Metal1MinEnclosureVia3 - _DRCObj._VIAxMinWidth) / (
        ...
        if _NumViaNMOSMet12Met2CoY == None:  # Default : calculate
            ...
            NumViaYNMOS = int((YWidthOfNMOSMet1 - 2 * _DRCObj._Metal1MinEnclosureVia3 - _DRCObj._VIAxMinWidth) / (
                        _DRCObj._VIAxMinWidth + _DRCObj._VIAxMinSpace)) + 1
        else:
        ...
        if _Finger in (1, 2):
            self._DesignParameter['_VIAPoly2Met1_F1'] = self._SrefElementDeclaration(
                _DesignObj=ViaPoly2Met1_resize._ViaPoly2Met1_resize(
                    _Name='ViaPoly2Met1_F1OnNMOSGateIn{}'.format(_Name)))[0]
        ...
        DistanceBtwVSS2NMOS2 = 0.5 * (self.getYWidth('PbodyContact', '_ODLayer') + self.getYWidth('_NMOS',
                                                                                                  '_ODLayer')) + _DRCObj._OdMinSpace  # OD Layer(for Pbody) - OD Layer (for NMOS)     OD=RX
        if '_PODummyLayer' in self._DesignParameter['_NMOS']['_DesignObj']._DesignParameter:
            DistanceBtwVSS2NMOS3 = 0.5 * (self.getYWidth('PbodyContact', '_ODLayer') + self.getYWidth('_NMOS',
                                                                                                      '_PODummyLayer')) + _DRCObj._PolygateMinSpace2OD  # OD Layer(for Pbody) - PO Dummy Layer (for NMOS)     OD=RX
        else:
            DistanceBtwVSS2NMOS3 = 0
        if '_Met1Layer' in self._DesignParameter['PbodyContact']['_DesignObj']._DesignParameter:
            DistanceBtwVSS2NMOS4 = 0.5 * (self.getYWidth('PbodyContact', '_Met1Layer') + self.getYWidth('_NMOS',
                                                                                                        '_Met1Layer')) + _DRCObj._Metal1MinSpace2  # Need to Modify
        else:
            DistanceBtwVSS2NMOS4 = 0
        DistanceBtwVSS2NMOS5 = 0.5 * (self.getYWidth('PbodyContact', '_PPLayer') + self.getYWidth('_NMOS',
                                                                                                  '_ODLayer')) + _DRCObj._OdMinSpace2Pp
        if _Finger in (1, 2):  # there is no _Met2Layer when finger is 1 or 2.
            DistanceBtwVSS2NMOS6 = 0
        else:
        #    DistanceBtwVSS2NMOS6 = 0
            DistanceBtwVSS2NMOS6 = 0.5 * (
                        self.getYWidth('PbodyContact', '_Met2Layer') + self.getYWidth('_ViaMet12Met2OnNMOSOutput',
                                                                                      '_Met2Layer')) + _DRCObj._MetalxMinSpace21
        DistanceBtwVSS2NMOS_minimum = max(DistanceBtwVSS2NMOS1, DistanceBtwVSS2NMOS2, DistanceBtwVSS2NMOS3,
                                          DistanceBtwVSS2NMOS4, DistanceBtwVSS2NMOS5, DistanceBtwVSS2NMOS6)

        DistanceBtwVDD2PMOS1 = 0.5 * (self.getYWidth('_PMOS', '_ODLayer') + self.getYWidth('NbodyContact',
                                                                                           '_ODLayer')) + _DRCObj._OdMinSpace
        DistanceBtwVD2PMOS2 = 0.5 * (self.getYWidth('NbodyContact', '_ODLayer') + self.getYWidth('_PMOS',
                                                                                                 '_ODLayer')) + _DRCObj._OdMinSpace  # OD Layer(for Nbody) - OD Layer (for PMOS)     OD=RX

        if '_PODummyLayer' in self._DesignParameter['_PMOS']['_DesignObj']._DesignParameter:
            DistanceBtwVDD2PMOS3 = 0.5 * (self.getYWidth('NbodyContact', '_ODLayer') + self.getYWidth('_PMOS',
                                                                                                      '_PODummyLayer')) + _DRCObj._PolygateMinSpace2OD  # OD Layer(for Nbody) - PO Dummy Layer (for PMOS)     OD=RX
        else:
            DistanceBtwVDD2PMOS3 = 0
        if '_Met1Layer' in self._DesignParameter['NbodyContact']['_DesignObj']._DesignParameter:
            DistanceBtwVDD2PMOS4 = 0.5 * (self.getYWidth('NbodyContact', '_Met1Layer') + self.getYWidth('_PMOS',
                                                                                                        '_Met1Layer')) + _DRCObj._Metal1MinSpace2  # Need to Modify
        else:
            DistanceBtwVDD2PMOS4 = 0
        DistanceBtwVDD2PMOS5 = 0.5 * (self.getYWidth('NbodyContact', '_ODLayer') + self.getYWidth('_PMOS',
                                                                                                  '_PPLayer')) + _DRCObj._OdMinSpace2Pp
        if _Finger in (1, 2):  # there is no _Met2Layer when finger is 1 or 2.
            DistanceBtwVDD2PMOS6 = 0
        else:
         #   DistanceBtwVDD2PMOS6 = 0 #delete met2
            DistanceBtwVDD2PMOS6 = 0.5 * (
                        self.getYWidth('NbodyContact', '_Met2Layer') + self.getYWidth('_ViaMet12Met2OnPMOSOutput',
                                                                                      '_Met2Layer')) + _DRCObj._MetalxMinSpace21
        DistanceBtwVDD2PMOS_minimum = max(DistanceBtwVDD2PMOS1, DistanceBtwVD2PMOS2, DistanceBtwVDD2PMOS3,
                                          DistanceBtwVDD2PMOS4, DistanceBtwVDD2PMOS5, DistanceBtwVDD2PMOS6)
        ...
        if _Finger not in (1, 2):
            DistanceBtwNMOS2PolyInput_byMet1 = \
                0.5 * max(self.getYWidth('_NMOS', '_Met1Layer'),
                          self.getYWidth('_ViaMet12Met2OnNMOSOutput', '_Met1Layer')) \
            ...
            topVia1Center_2_NMOS = CoordCalc.getXYCoords_MaxY(self.getXY('_ViaMet12Met2OnNMOSOutput', '_COLayer'))[0][
                                       1] - self.getXY('_ViaMet12Met2OnNMOSOutput')[0][
                                       1]  # calc by temporally set Via coordinates =[[0,0]]
            distanceBtwNMOS2PolyInput_byVia1Contact = topVia1Center_2_NMOS + (
            ...
            DistanceBtwNMOS2PolyInput = max(DistanceBtwNMOS2PolyInput_byMet1, DistanceBtwNMOS2PolyInput_byODandPoly,
                                            distanceBtwNMOS2PolyInput_byVia1Contact)

            DistanceBtwPMOS2PolyInput_byMet1 = \
                0.5 * max(self.getYWidth('_PMOS', '_Met1Layer'),
                          self.getYWidth('_ViaMet12Met2OnPMOSOutput', '_Met1Layer')) \
                + 0.5 * WidthOfInputXM1 + _DRCObj._Metal1MinSpaceAtCorner
            DistanceBtwPMOS2PolyInput_byODandPoly = \
                ...
                + 0.5 * (_DRCObj._CoMinWidth + 2 * _DRCObj._CoMinEnclosureByPOAtLeastTwoSide) + tmpDRC_ODandPoly
            #
            botVia1Center_2_PMOS = self.getXY('_ViaMet12Met2OnPMOSOutput')[0][1] - \
                                   CoordCalc.getXYCoords_MinY(self.getXY('_ViaMet12Met2OnPMOSOutput', '_COLayer'))[0][
                                       1]  # calc by temporally set Via coordinates =[[0,0]]
            distanceBtwPMOS2PolyInput_byVia1Contact = botVia1Center_2_PMOS + (
                        _DRCObj._VIAxMinSpace2 + _DRCObj._VIAxMinWidth)  ##
            DistanceBtwPMOS2PolyInput = max(DistanceBtwPMOS2PolyInput_byMet1, DistanceBtwPMOS2PolyInput_byODandPoly,
                                            distanceBtwPMOS2PolyInput_byVia1Contact)

        else:  # Finger == 1, 2
            XCoordinateOfViaF1 = \
            self._DesignParameter['_PMOS']['_DesignObj']._DesignParameter['_XYCoordinatePMOSSupplyRouting'][
                '_XYCoordinates'][0][0] + (_ChannelLength + _GateSpacing) - \
            self._DesignParameter['_PMOS']['_DesignObj']._DesignParameter['_Met1Layer'][
                '_XWidth'] / 2 - 60 - self.getXWidth('_VIAPoly2Met1_F1', '_Met1Layer') / 2
            _XGapPolyDummyEdge2PolyEdge = \
                (XCoordinateOfViaF1 - 0.5 * self.getXWidth('_VIAPoly2Met1_F1', '_POLayer')) \
                - (self._DesignParameter['_NMOS']['_DesignObj']._DesignParameter['_PODummyLayer']['_XYCoordinates'][0][
                       0] + 0.5 * self.getXWidth('_NMOS', '_PODummyLayer'))
            _YGapPolyDummy2InputViaPoly = math.ceil(
                math.sqrt(_DRCObj._PolygateMinSpaceAtCorner ** 2 - _XGapPolyDummyEdge2PolyEdge ** 2))
        ...
        if _VDD2VSSHeight == None:
            ...
            if _VDD2VSSHeight < _VDD2VSSMinHeight:
                raise NotImplementedError(
                    f"ERROR! VDD2VSSMinHeight={_VDD2VSSMinHeight}, but input _VDD2VSSHeight is {_VDD2VSSHeight}")
        ...
        if (_Finger in (1, 2)) and (DesignParameters._Technology == 'SS28nm'):
            botYCoordOfPMOSPoly = self._DesignParameter['_PMOS']['_XYCoordinates'][0][1] - self.getYWidth('_PMOS',
                                                                                                          '_PODummyLayer') / 2
            topYCoordOfPMOSPoly = self._DesignParameter['_NMOS']['_XYCoordinates'][0][1] + self.getYWidth('_NMOS',
                                                                                                          '_PODummyLayer') / 2
            ...
            # error check
            xDistance = self.getXYLeft('_VIAPoly2Met1_F1', '_POLayer')[0][0] - \
                        self.getXYRight('_NMOS', '_PODummyLayer')[0][0]
            yDistance1 = abs(
                self.getXYTop('_VIAPoly2Met1_F1', '_POLayer')[0][1] - self.getXYBot('_PMOS', '_PODummyLayer')[0][1])
            yDistance2 = abs(
                self.getXYBot('_VIAPoly2Met1_F1', '_POLayer')[0][1] - self.getXYTop('_NMOS', '_PODummyLayer')[0][1])
        ...
        if _Finger not in (1, 2):
            ...
            _LenBtwPMOSGates = abs(
                self.getXY('_VIAPoly2Met1_F1')[0][0] - self.getXY('_PMOS', '_XYCoordinatePMOSGateRouting')[-1][0]) \
                               + self.getXWidth('_VIAPoly2Met1_F1', '_POLayer') / 2 \
                               + self.getXWidth('_PMOS', '_POLayer') / 2
            _LenBtwNMOSGates = abs(
                self.getXY('_VIAPoly2Met1_F1')[0][0] - self.getXY('_NMOS', '_XYCoordinateNMOSGateRouting')[-1][0]) \
                               + self.getXWidth('_VIAPoly2Met1_F1', '_POLayer') / 2 \
        ...
        self._DesignParameter['_PolyRouteXOnPMOS'][
            '_YWidth'] = YWidthOf_PolyRouteXOnPMOS  # calculated when only one row -> should be changed
        ...
        if _Finger not in (1, 2):
            ...
            self._DesignParameter['_PolyRouteXOnNMOS']['_XYCoordinates'] = [
                [self.getXY('_NMOS', '_POLayer')[-1][0] + _ChannelLength / 2 - _LenBtwNMOSGates / 2,
                 self.getXY('_VIAPoly2Met1_F1', '_POLayer')[0][1] - self.getYWidth('_VIAPoly2Met1_F1',
                                                                                   '_POLayer') / 2 + YWidthOf_PolyRouteXOnPMOS / 2]
            ]
        ...
        if _Finger in (1, 2):
            ...
            for i in range(0, (len(
                    self._DesignParameter['_PMOS']['_DesignObj']._DesignParameter['_XYCoordinatePMOSOutputRouting'][
                        '_XYCoordinates'])) // 2):
                tmpOutputRoutingM1Y.append([CoordCalc.Add(
                    self.getXY('_PMOS', '_XYCoordinatePMOSOutputRouting')[-1 - 2 * i],
                    [0, +self.getYWidth('_PMOS', '_Met1Layer') / 2]),
                                            CoordCalc.Add(
                                                self.getXY('_NMOS', '_XYCoordinateNMOSOutputRouting')[-1 - 2 * i],
                                                [0, -self.getYWidth('_NMOS', '_Met1Layer') / 2])
                                            ])  #
        ...
        if _Finger not in (1, 2):
            ...
            distance_poly = distance_input - max(self._DesignParameter['_PolyRouteXOnPMOS']['_YWidth'],
                                                 self._DesignParameter['_VIATempPoly2Met1'][
            ...
            if (not flag_contact) and (not flag_metal) and (not flag_poly):
                ...
                self._DesignParameter['_PolyRouteXOnMOS']['_XWidth'] = self._DesignParameter['_PolyRouteXOnPMOS'][
                    '_XWidth']
                self._DesignParameter['_PolyRouteXOnMOS']['_YWidth'] = self._DesignParameter['_PolyRouteXOnPMOS'][
                    '_YWidth']
        ...
        if _Finger not in (1, 2):
            ...
            tmpLeftPOBoundary = \
            self._DesignParameter['_PMOS']['_DesignObj']._DesignParameter['_POLayer']['_XYCoordinates'][0][
                0] - 0.5 * _ChannelLength
            ...
            NumCoLeftMost = int((tmpRightCoBoundary - tmpLeftCoBoundary - _DRCObj._CoMinWidth) // (
            ...
            if NumCoLeftMost > 1:
                ...
                self._DesignParameter['_VIAMOSPoly2Met1LeftMost']['_DesignObj']._CalculateViaPoly2Met1DesignParameter(
                    **_VIAMOSPoly2Met1LeftMost)
            ...
            if len(self._DesignParameter['_OutputRouting']['_XYCoordinates']) > 1:
                ...
                for i in range(0, len(self._DesignParameter['_OutputRouting']['_XYCoordinates']) - 1):
                    tmpInputPCCOM1onNMOS.append(
                        [(self._DesignParameter['_OutputRouting']['_XYCoordinates'][i][0][0] +
                          self._DesignParameter['_OutputRouting']['_XYCoordinates'][i + 1][0][0]) / 2,
                         self._DesignParameter['_PolyRouteXOnNMOS']['_XYCoordinates'][0][1]])
        ...
        if '_VIAMOSPoly2Met1LeftMost' in self._DesignParameter:
            ...
            tmpInputRouting.append([
                [self.getXYLeft('_VIAMOSPoly2Met1LeftMost', '_Met1Layer')[0][0] + self.getWidth('_InputRouting') / 2,
                 self.getXY('_VIAMOSPoly2Met1LeftMost')[0][1]],
                [self.getXYLeft('_VIAMOSPoly2Met1LeftMost', '_Met1Layer')[1][0] + self.getWidth('_InputRouting') / 2,
                 self.getXY('_VIAMOSPoly2Met1LeftMost')[1][1]],
            ])
        ...
        if (tmpCount_GateViaLayer > 1) and flag_horizontal_inputvia_PCCOM1:
            ...
            # (1) LeftMost
            _LengthM1_VIAPMOSPoly2Met1_LeftMost = \
            self._DesignParameter['_VIAMOSPoly2Met1LeftMost']['_DesignObj']._DesignParameter['_Met1Layer']['_XWidth']
            _NumViaInputM12_LeftMost = int((
                                                       _LengthM1_VIAPMOSPoly2Met1_LeftMost - _DRCObj._VIAxMinWidth - 2 * _DRCObj._VIAxMinEnclosureByMetxTwoOppositeSide)
                                           // (_DRCObj._VIAxMinWidth + _DRCObj._VIAxMinSpace)) + 1
            ...
            self._DesignParameter['_ViaMet12Met2forInput2']['_DesignObj']._CalculateDesignParameterSameEnclosure(
                **_ViaMet12Met2forInput2)

            if _NumViaInputM12_LeftMost == 1:
                self._DesignParameter['_ViaMet12Met2forInput2']['_XYCoordinates'] = \
                    [[self.getXYRight('_VIAMOSPoly2Met1LeftMost', '_Met1Layer')[0][0] - self.getXWidth(
                        '_ViaMet12Met2forInput2', '_Met1Layer') / 2,
                      self._DesignParameter['_VIAMOSPoly2Met1LeftMost']['_XYCoordinates'][0][1]]]

                leftBoundary = min(self.getXYLeft('_ViaMet12Met2forInput2', '_Met1Layer')[0][0],
                                   self.getXYLeft('_VIAMOSPoly2Met1LeftMost', '_Met1Layer')[0][0])
                rightBoundary = max(self.getXYRight('_ViaMet12Met2forInput2', '_Met1Layer')[0][0],
                                    self.getXYRight('_VIAMOSPoly2Met1LeftMost', '_Met1Layer')[0][0])
                self._DesignParameter['Met1Boundarytmp'] = self._BoundaryElementDeclaration(
                    _Layer=DesignParameters._LayerMapping['METAL1'][0],
                    _Datatype=DesignParameters._LayerMapping['METAL1'][1],
                    _XWidth=(rightBoundary - leftBoundary),
                    _YWidth=max(self.getYWidth('_ViaMet12Met2forInput2', '_Met1Layer'),
                                self.getYWidth('_VIAMOSPoly2Met1LeftMost', '_Met1Layer')),
            ...
            if '_VIAPMOSPoly2Met1' in self._DesignParameter:
                ...
                self._DesignParameter['_ViaMet12Met2forInput']['_DesignObj']._CalculateDesignParameterSameEnclosure(
                    **_ViaMet12Met2forInput)
            ...
            #     _Layer=DesignParameters._LayerMapping['METAL2'][0],
            #     _Datatype=DesignParameters._LayerMapping['METAL2'][1])
            # self._DesignParameter['_CLKMet2InRouting'][
            #     '_Width'] = _DRCObj._VIAxMinWidth + 2 * _DRCObj._Metal1MinEnclosureVia3
            # self._DesignParameter['_CLKMet2InRouting']['_XYCoordinates'] = [
        ...
        if _SupplyRailType == 2:
            tmpXYs_VDDVia = []
            tmpXYs_VSSVia = []
            for XYs in self._DesignParameter['_PMOS']['_DesignObj']._DesignParameter['_XYCoordinatePMOSSupplyRouting'][
                '_XYCoordinates']:
                tmpXYs_VDDVia.append([XYs[0], _VDD2VSSHeight])
            for XYs in self._DesignParameter['_NMOS']['_DesignObj']._DesignParameter['_XYCoordinateNMOSSupplyRouting'][
                '_XYCoordinates']:
        ...
        if flag_horizontal_inputvia_PCCOM1:  # finger not in (1, 2) && merging input contact
            self._DesignParameter['InputMet1']['_XWidth'] = self.getXWidth('_VIAMOSPoly2Met1LeftMost',
                                                                           '_Met1Layer')  # before, _VIANMOSPoly2Met1
            ...
            self._DesignParameter['InputMet1']['_XYCoordinates'] = [
                [self._DesignParameter['_InputRouting']['_XYCoordinates'][0][0][0],
                 (self._DesignParameter['_InputRouting']['_XYCoordinates'][0][0][1] +
                  self._DesignParameter['_InputRouting']['_XYCoordinates'][0][1][1]) / 2]
            ]
        ...
        for ii in range(0, Num_DRCCheck):
            ...
            for iii in range(0, forLoopCntMax):
                try:
                    ...
                    tmpObj = _Inverter(_Name=cellname)
                    InputParamsTT = {}
                    InputParamsTT['_Finger'] = InputParams['NumFinger']
                    InputParamsTT['_ChannelWidth'] = InputParams['NMOSWidth']
                    InputParamsTT['_ChannelLength'] = InputParams['ChannelLength']
                    InputParamsTT['_NPRatio'] = InputParams['PMOSWidth'] / InputParams['NMOSWidth']
                    InputParamsTT['_Dummy'] = True
                    InputParamsTT['_XVT'] = InputParams['XVT']
                    InputParamsTT['_GateSpacing'] = InputParams['GateSpacing']
                    InputParamsTT['_SDWidth'] = InputParams['SDWidth']
                    InputParamsTT['_SupplyRailType'] = InputParams['SupplyRailType']

                    tmpCellHeigth = tmpObj._CalcMinHeight(**InputParamsTT)
                    InputParams['CellHeight'] = DRCchecker.RandomParam(start=tmpCellHeigth, stop=tmpCellHeigth + 1000,
                                                                       step=10)
                    ...
                    LayoutObj._UpdateDesignParameter2GDSStructure(
                        _DesignParameterInDictionary=LayoutObj._DesignParameter)
