...
class INVERTER_CHAIN(StickDiagram._StickDiagram):
    ...
    def __init__(self, _DesignParameter=None, _Name=f'MS_inverter_chain'):
    ...
    def _CalculateDesignParameter(self,
        ...
        inv2_x = inv1_x + abs(self._DesignParameter['inv1']['_DesignObj'].rightmost_poly_edge) + \
                 abs(self._DesignParameter['inv2'][
                         '_DesignObj'].leftmost_poly_edge) + drc._PolygateMinSpace
        inv3_x = inv2_x + abs(self._DesignParameter['inv2']['_DesignObj'].rightmost_poly_edge) + \
                 abs(self._DesignParameter['inv3'][
                         '_DesignObj'].leftmost_poly_edge) + drc._PolygateMinSpace
        ...
        """
        Via 23 for moscap_coarse input Generation
        """
        self._DesignParameter['via_for_moscap_coarse'] = \
        self._SrefElementDeclaration(_DesignObj=ViaMet22Met3._ViaMet22Met3(
            _Name='via_for_moscap_coarse_in_{}'.format(_Name)))[0]
        via_inputs = copy.deepcopy(ViaMet22Met3._ViaMet22Met3._ParametersForDesignCalculation)
        via_inputs['_ViaMet22Met3NumberOfCOX'] = 2
        via_inputs['_ViaMet22Met3NumberOfCOY'] = 1
        self._DesignParameter['via_for_moscap_coarse'][
            '_DesignObj']._CalculateViaMet22Met3DesignParameterMinimumEnclosureY(
            **via_inputs)
        x_value_23 = self.getXYLeft('inv1', 'pmos', 'pmos_gate_via', '_Met1Layer')[0][0] - \
                   self.getXWidth('via_for_moscap_coarse', '_Met2Layer') / 2
        y_value_23 = self.getXYTop('inv1', 'pmos', 'pmos_gate_via', '_Met1Layer')[0][1] - \
                   self.getYWidth('via_for_moscap_coarse', '_Met2Layer') / 2
        # tmp_xy = self.getXYTop('inv3', 'nmos_output_via', '_Met2Layer')[-1]
        # calibre_value = self.getYWidth('via_for_moscap_fine', '_Met3Layer') / 2
        via_xy = [[x_value_23, y_value_23]]
        self._DesignParameter['via_for_moscap_coarse']['_XYCoordinates'] = via_xy
        ...
        x_value1 = self.getXYLeft('inv2', 'pmos', 'pmos_gate_via', '_Met1Layer')[0][0] - \
                   self.getXWidth('via_12_array', '_Met1Layer') / 2
        x_value2 = self.getXYLeft('inv3', 'pmos', 'pmos_gate_via', '_Met1Layer')[0][0] - \
                   self.getXWidth('via_12_array', '_Met1Layer') / 2
        #
        # x_value3 = self.getXYRight('inv3', 'nmos', 'nmos_gate_via', '_Met1Layer')[0][0] + \
        #            self.getXWidth('via_12_array', '_Met1Layer') / 2
        # y_value_for_3 = self.getXYBot('inv3', 'nmos', 'nmos_gate_via', '_Met1Layer')[0][1] + \
        #            self.getYWidth('via_12_array', '_Met1Layer') / 2

        via_xy = [[x_value_23, y_value_23], [x_value1, common_y], [x_value2, common_y]]
        self._DesignParameter['via_12_array']['_XYCoordinates'] = via_xy


        """
        pmos_output_input_routing
        """
        self._DesignParameter['pmos_12_routing'] = self._PathElementDeclaration(
            _Layer=DesignParameters._LayerMapping['METAL2'][0],
            _Datatype=DesignParameters._LayerMapping['METAL2'][1],
            _Width=drc._MetalxMinWidth)

        xy_source1 = self.getXY('inv1', 'pmos_output_via', '_Met2Layer')[-1]
        x_value_target1 = x_value1
        xy_source2 = self.getXY('inv2', 'pmos_output_via', '_Met2Layer')[-1]
        x_value_target2 = x_value2

        path_xy = []
        center_to_edge_x_value1 = xy_source1[0] - self.getXY('inv1', 'pmos', 'pmos_gate_via', '_Met1Layer')[0][0]
        edge_to_routing_y_value1 = self.getXYBot('inv1', 'pmos_output_via', '_Met2Layer')[-1][1] - common_y + \
                                   drc._MetalxMinWidth / 2

        center_to_edge_x_value2 = xy_source2[0] - self.getXY('inv2', 'pmos', 'pmos_gate_via', '_Met1Layer')[0][0]
        edge_to_routing_y_value2 = self.getXYBot('inv2', 'pmos_output_via', '_Met2Layer')[-1][1] - common_y + \
                                   drc._MetalxMinWidth / 2

        if center_to_edge_x_value1 < edge_to_routing_y_value1:
            path_xy.append([[self.getXY('inv1', 'pmos', 'pmos_gate_via', '_Met1Layer')[0][0],common_y],
                            [x_value_target1, common_y]])
        else:
            path_xy.append([xy_source1,[xy_source1[0],common_y],[x_value_target1, common_y]])

        if center_to_edge_x_value2 < edge_to_routing_y_value2:
            path_xy.append([[self.getXY('inv2', 'pmos', 'pmos_gate_via', '_Met1Layer')[0][0], common_y],
                            [x_value_target1, common_y]])
        else:
            path_xy.append([xy_source2, [xy_source2[0], common_y], [x_value_target2, common_y]])


        self._DesignParameter['pmos_12_routing']['_XYCoordinates'] = path_xy

        """
        Values for upper cells
        """

        self.space_bw_np_gate_centers = self.getXY('inv1','pmos', 'pmos_gate_via','_Met1Layer')[0][1] - \
                                        self.getXY('inv1','nmos', 'nmos_gate_via', '_Met1Layer')[0][1]

        self.distance_to_vdd = self.getXY('inv1','vdd', '_Met1Layer')[0][1]
        self.distance_to_vss = abs(self.getXY('inv1','vss', '_Met1Layer')[0][1])

        self.leftmost_poly_edge = \
            min(self.getXYLeft('inv1','nmos','nmos','_PODummyLayer')[0][0],
                                      self.getXYLeft('inv1','pmos','pmos', '_PODummyLayer')[0][0])
        self.rightmost_poly_edge = max(self.getXYRight('inv3','nmos','nmos','_PODummyLayer')[-1][0],
                                      self.getXYRight('inv3','pmos','pmos', '_PODummyLayer')[-1][0])
...
if __name__ == '__main__':
    ...
    for i in range(0, 1):
        ...
        _fileName = 'MS_inverter_chain.gds'
        testStreamFile = open('./MS_inverter_chain.gds', 'wb')
        ...
        myfile = open('MS_inverter_chain.gds', 'rb')
        ftp.storbinary('STOR MS_inverter_chain.gds', myfile)
        myfile.close()
        ftp.close()
