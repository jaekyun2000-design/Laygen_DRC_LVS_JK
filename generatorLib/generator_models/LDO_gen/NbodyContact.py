...
class _NbodyContact(StickDiagram._StickDiagram):
    _ParametersForDesignCalculation = dict(_InputModeArea=True, _NumberOfNbodyCOX=None, _NumberOfNbodyCOY=None,
                                           _Met1XWidth=None, _Met1YWidth=None, _COXpitch=None, _COYpitch=None,
                                           _NWEnclosure=None)

    '''
        _InputModeArea = True -> Generate NbodyContact by _Met1XWidth & _Met1YWidth
        _InputModeArea = False -> Generate NbodyContact by _NumberOfNbodyCOX & _NumberOfNbodyCOY

        _COXpitch & _COYpitch : Can define CONT layer's pitch by users
        _NWEnclosure : Can define gap between PIMP(BP) layer and Metal1 layer
    '''
    ...
    def _CalculateNbodyContactDesignParameter(self, _InputModeArea=True, _NumberOfNbodyCOX=None, _NumberOfNbodyCOY=None, _Met1XWidth=None, _Met1YWidth=None, _COXpitch=None, _COYpitch=None, _NWEnclosure=None):
        print('#########################################################################################################')
        ...
        if _COXpitch == None:
            _XpitchBtwCO = _DRCObj._CoMinWidth + _DRCObj.DRCCOMinSpace(NumOfCOX=_NumberOfNbodyCOX, NumOfCOY=_NumberOfNbodyCOY)
        else:
            _XpitchBtwCO = _COXpitch

        if _COYpitch == None:
            _YpitchBtwCO = _DRCObj._CoMinWidth + _DRCObj.DRCCOMinSpace(NumOfCOX=_NumberOfNbodyCOX, NumOfCOY=_NumberOfNbodyCOY)
        else:
        ...
        if _InputModeArea == True:
            ...
            self._DesignParameter['_NWLayer']['_XYCoordinates'] = _XYCoordinateOfNbodyContact
            if _NWEnclosure != None:
                self._DesignParameter['_NWLayer']['_XWidth'] = _Met1XWidth + _NWEnclosure * 2
                self._DesignParameter['_NWLayer']['_YWidth'] = _Met1YWidth + _NWEnclosure * 2

            else:
                self._DesignParameter['_NWLayer']['_XWidth'] = self._DesignParameter['_ODLayer']['_XWidth'] + 2 * _DRCObj._NwMinEnclosureNactive
                self._DesignParameter['_NWLayer']['_YWidth'] = self._DesignParameter['_ODLayer']['_YWidth'] + 2 * _DRCObj._NwMinEnclosureNactive
            ...
            for i in range(0, _NumofCOX):
                for j in range(0, _NumofCOY):
                    if (_NumofCOX % 2) == 0 and (_NumofCOY % 2) == 0:
                        _xycoordinatetmp = [_XYCoordinateOfNbodyContact[0][0] - (_NumofCOX // 2 - 0.5) * _XpitchBtwCO + i * _XpitchBtwCO,
                                            _XYCoordinateOfNbodyContact[0][1] - (_NumofCOY // 2 - 0.5) * _YpitchBtwCO + j * _YpitchBtwCO]

                    elif (_NumofCOX % 2) == 0 and (_NumofCOY % 2) == 1:
                        _xycoordinatetmp = [_XYCoordinateOfNbodyContact[0][0] - (_NumofCOX // 2 - 0.5) * _XpitchBtwCO + i * _XpitchBtwCO,
                                            _XYCoordinateOfNbodyContact[0][1] - (_NumofCOY - 1) // 2 * _YpitchBtwCO + j * _YpitchBtwCO]

                    elif (_NumofCOX % 2) == 1 and (_NumofCOY % 2) == 0:
                        _xycoordinatetmp = [_XYCoordinateOfNbodyContact[0][0] - (_NumofCOX - 1) // 2 * _XpitchBtwCO + i * _XpitchBtwCO,
                                            _XYCoordinateOfNbodyContact[0][1] - (_NumofCOY // 2 - 0.5) * _YpitchBtwCO + j * _YpitchBtwCO]
            ...
            self._DesignParameter['_ODLayer']['_XYCoordinates'] = _XYCoordinateOfNbodyContact
            if _Met1XWidth == None:
                self._DesignParameter['_ODLayer']['_XWidth'] = _DRCObj._CoMinWidth + (_NumberOfNbodyCOX - 1) * _XpitchBtwCO + 2 * _DRCObj._CoMinEnclosureByODAtLeastTwoSide
            else:
                self._DesignParameter['_ODLayer']['_XWidth'] = _Met1XWidth
            if _Met1YWidth == None:
                self._DesignParameter['_ODLayer']['_YWidth'] = _DRCObj._CoMinWidth + (_NumberOfNbodyCOY - 1) * _YpitchBtwCO + 2 * _DRCObj._CoMinEnclosureByODAtLeastTwoSide
            else:
                self._DesignParameter['_ODLayer']['_YWidth'] = _Met1YWidth
            ...
            self._DesignParameter['_NWLayer']['_XYCoordinates'] = _XYCoordinateOfNbodyContact
            if _NWEnclosure != None:
                self._DesignParameter['_NWLayer']['_XWidth'] = self._DesignParameter['_ODLayer']['_XWidth'] + _NWEnclosure * 2
                self._DesignParameter['_NWLayer']['_YWidth'] = self._DesignParameter['_ODLayer']['_YWidth'] + _NWEnclosure * 2
            ...
            _NumofCOX = _NumberOfNbodyCOX
            _NumofCOY = _NumberOfNbodyCOY
            ...
            for i in range(0, _NumofCOX):
                for j in range(0, _NumofCOY):
                    if (_NumofCOX % 2) == 0 and (_NumofCOY % 2) == 0:
                        ...
                        _xycoordinatetmp = [_XYCoordinateOfNbodyContact[0][0] - (_NumofCOX - 1) // 2 * _XpitchBtwCO + i * _XpitchBtwCO,
                                            _XYCoordinateOfNbodyContact[0][1] - (_NumofCOY - 1) // 2 * _XpitchBtwCO + j * _XpitchBtwCO]
                    tmp.append(_xycoordinatetmp)
...
#     NbodyContactObj._CalculateNbodyContactDesignParameter(_InputModeArea=True, _NumberOfNbodyCOX=3,
#                                                           _NumberOfNbodyCOY=58, _Met1XWidth=348, _Met1YWidth=8272,
#                                                           _COXpitch=142, _COYpitch=142, _NWEnclosure=56)
#     NbodyContactObj._UpdateDesignParameter2GDSStructure(_DesignParameterInDictionary=NbodyContactObj._DesignParameter)
#     _fileName = 'NbodyContact.gds'
#     testStreamFile = open('./NbodyContact.gds', 'wb')
...
#     myfile = open('NbodyContact.gds', 'rb')
#     ftp.storbinary('STOR NbodyContact.gds', myfile)
#     myfile.close()