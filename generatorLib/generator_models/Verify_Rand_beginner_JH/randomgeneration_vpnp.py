...
from generatorLib.generator_models import vpnp_
...
class RandGen:
    ...
    def create_layout(self, params, word='1'):
        ...
        if self.cell_name == 'vpnp':
            ...
            vpnp = vpnp_.vpnp(_DesignParameter=None, _Name='vpnp')
            ...
            vpnp._CalculateDesignParameter(W=params.get('W'))
            # inv._UpdateDesignParameter2GDSStructure(_DesignParameterInDictionary=inv._DesignParameter)
            vpnp._UpdateDesignParameter2GDSStructure(_DesignParameterInDictionary=vpnp._DesignParameter)
            ...
            with open(f'./Verify_Rand_beginner_JH/vpnp/vpnp{word}.gds', 'wb') as f:
                gds_stream = vpnp._CreateGDSStream(vpnp._DesignParameter['_GDSFile']['_GDSFile'])
                gds_stream.write_binary_gds_stream(f)
...
if __name__=="__main__":
    ...
    randgen = RandGen(cell_name='vpnp',
                      W=(100, [1100, 22000])
                      )
    ...
    drc_result, lvs_result = randgen.run(10)
