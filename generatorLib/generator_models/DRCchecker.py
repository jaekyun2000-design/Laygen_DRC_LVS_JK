...
class DRCchecker:
    ...
    def DRCchecker(self):
        if DesignParameters._Technology == 'SS28nm':
            ...
            _Technology = DesignParameters._Technology
        ...
        if _Technology == '065nm':
            ...
            Techlib = 'tsmcN65'
        if _Technology == '045nm':
            DRCfile = '_calibre.drc_'
            Techlib = 'tsmcN45'
        if _Technology == '090nm':
            DRCfile = '_calibre.drc_'
            Techlib = 'tsmcN90rf'
        ...
        stdin, stdout, stderr = ssh.exec_command(
            commandlines1.format(self.WorkDir, self.libname, self.cellname, self.GDSDir, Techlib))
    ...
    def StreamIn(self, tech: str = '028nm'):
        ...
        stdin, stdout, stderr = ssh.exec_command(
            commandlines1.format(self.WorkDir, self.libname, self.GDSDir, filename, TechFile))
        result1 = stdout.read().decode('utf-8')

        print('   Stream In   '.center(105, '-'))
        print(result1)
        if (result1.split()[
            -6]) != "'0'":  # Example of result1's Last Line : INFO (XSTRM-234): Translation completed. '0' error(s) and '125' warning(s) found.
            raise Exception("Library name already Existing or XStream ERROR!!")
