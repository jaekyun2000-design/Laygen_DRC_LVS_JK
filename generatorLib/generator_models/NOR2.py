...
class NOR2(StickDiagram._StickDiagram):
    ...
    def _CalculateDesignParameter(self, CellHeight=1800, YCoordOfNMOS=400, YCoordOfPMOS=1400, YCoordOfInputOutput=900,
                                  ChannelLength=30, GateSpacing=100, XVT='SLVT',
                                  NMOSWidth=200, PMOSWidth=400, NumFinger_NM=2, NumFinger_PM=4):
        ...
        #     _Datatype=DesignParameters._LayerMapping['METAL1'][1],
        #     _XWidth=self.getXWidth('Met1RouteY_PMOut'),
        #     _YWidth=(self.getXY('Met1RouteY_PMOut')[0][1] + self.getYWidth('Met1RouteY_PMOut') / 2) - (self.getXY('Met1RouteX_NMout')[0][1] - self.getYWidth('Met1RouteX_NMout') / 2)
        # )
        # self._DesignParameter['Met1RouteY_OutputPM2NM']['_XYCoordinates'] = [[
        #     self.getXY('Met1RouteX_PMOut')[0][0],
        #     ((self.getXY('Met1RouteY_PMOut')[0][1] + self.getYWidth('Met1RouteY_PMOut') / 2) + (self.getXY('Met1RouteX_NMout')[0][1] - self.getYWidth('Met1RouteX_NMout') / 2)) / 2
        # ]]
        #
        # self._DesignParameter['Met1RouteX_OutputPM2NM_2'] = self._BoundaryElementDeclaration(
        #     _Layer=DesignParameters._LayerMapping['METAL1'][0],
        #     _Datatype=DesignParameters._LayerMapping['METAL1'][1],
        #     _XWidth=(self.getXY('Met1RouteY_OutputPM2NM')[0][0] + self.getXWidth('Met1RouteY_OutputPM2NM') / 2) - (self.getXY('Met1RouteY_NMout')[0][0] - self.getXWidth('Met1RouteY_NMout') / 2),
        #     _YWidth=YWidth_Met1HorizontalRouting
        # )
        # self._DesignParameter['Met1RouteX_OutputPM2NM_2']['_XYCoordinates'] = [[
        #     ((self.getXY('Met1RouteY_OutputPM2NM')[0][0] + self.getXWidth('Met1RouteY_OutputPM2NM') / 2) + (self.getXY('Met1RouteY_NMout')[0][0] - self.getXWidth('Met1RouteY_NMout') / 2)) / 2,
        #     self.getXY('Met1RouteY_OutputPM2NM')[0][1] - self.getYWidth('Met1RouteY_OutputPM2NM') / 2 + self.getYWidth('Met1RouteX_OutputPM2NM_2') / 2
        # ]]
        #
        # self._DesignParameter['ViaPoly_InputAB'] = self._SrefElementDeclaration(_DesignObj=ViaPoly2Met1_resize._ViaPoly2Met1_resize(_Name='ViaPoly_InputABIn{}'.format(_Name)))[0]    # manually edited
        # self._DesignParameter['ViaPoly_InputAB']['_DesignObj']._CalculateDesignParameter(
        #     **dict(_ViaPoly2Met1NumberOfCOX=1, _ViaPoly2Met1NumberOfCOY=2,
        #            Met1XWidth=66, Met1YWidth=200, POXWidth=40, POYWidth=200))
        #
        # if max(NumFinger_NM, NumFinger_PM) == 1:
        #     XCoord_InputA = self.getXY('PMOS', '_Met1Layer')[0][0]
        # else:
        #     XCoord_InputA = - max(NumFinger_NM, NumFinger_PM) / 2 * (GateSpacing + ChannelLength)
        #
        # XCoord_InputB = self.getXY('Met1RouteY_OutputPM2NM')[0][0] - (GateSpacing + ChannelLength)
        # XYCoord_InputA = [XCoord_InputA, YCoordOfInputOutput]
        # XYCoord_InputB = [XCoord_InputB, YCoordOfInputOutput]
        # self._DesignParameter['ViaPoly_InputAB']['_XYCoordinates'] = [XYCoord_InputA, XYCoord_InputB]
