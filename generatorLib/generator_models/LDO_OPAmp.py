...
# from Verifier.DRCchecker import DRCchecker
...
start = time.time()
class LDO_OPAmp(StickDiagram._StickDiagram):

	def __init__(self, _DesignParameter=None, _Name='LDO_OPAmp'):
	...
	def _CalculateDesignParameter(self,
								  ...
								  ChannelLength=150,
								  MOS_Width=400,   # DRC: <1.211(when finger > 10) -> <=1190
								  GateSpacing=114,
								  SDWidth=50,
								  XVT='SLVT',

								  NCAP_XWidth=4300,  # Length
								  NCAP_YWidth=4550,  # Width
								  NCAP_NumofGate=2,
		...
		if Insert_comp == True:
			...
			# RHP zero Res
			self._DesignParameter['RES'] = self._SrefElementDeclaration(_Reflect=[0, 0, 0], _Angle=270, _DesignObj=opppcres_b_v2._Opppcres(_Name='RESIn{}'.format(_Name)))[0]
		...
		if Insert_comp == True:
			...
			if _SeriesStripes % 2 == 0:
				res_x_coord = \
				self._DesignParameter['RES']['_DesignObj']._DesignParameter['_Met1Layer']['_XYCoordinates'][1][1]
			else:
				res_x_coord = \
				self._DesignParameter['RES']['_DesignObj']._DesignParameter['_Met1Layer']['_XYCoordinates'][0][1]
			if (self.getXY('_NCAPMet2RXRouting')[0][0][1] + self.getWidth('_NCAPMet2RXRouting') / 2
				...
				self._DesignParameter['_NCAP2RESMet2Routing']['_XYCoordinates'] = [[self.getXY('_NCAPMet2RXRouting')[0][0],
																					[self._DesignParameter['RES']['_XYCoordinates'][0][0] + res_x_coord + self.getYWidth('RES', '_Met1Layer') / 2,#self._DesignParameter['RES']['_DesignObj']._DesignParameter['_Met1Layer']['_XYCoordinates'][1][1] + self.getYWidth('RES', '_Met1Layer') / 2,
				...
				self._DesignParameter['_NCAP2RESMet1Routing']['_XYCoordinates'] = [[[self._DesignParameter['RES']['_XYCoordinates'][0][0] + res_x_coord, #self._DesignParameter['RES']['_DesignObj']._DesignParameter['_Met1Layer']['_XYCoordinates'][-1][1],
																					 self.getXY('_NCAPMet2RXRouting')[0][0][1] - self.getWidth('_NCAPMet2RXRouting') / 2],
																					[self._DesignParameter['RES']['_XYCoordinates'][0][0] + res_x_coord, #self._DesignParameter['RES']['_DesignObj']._DesignParameter['_Met1Layer']['_XYCoordinates'][-1][1],
				...
				self._DesignParameter['_NCAP2RESMet2Routing']['_XYCoordinates'] = [[self.getXY('_NCAPMet2RXRouting')[0][0],
																					[self._DesignParameter['RES']['_XYCoordinates'][0][0] + res_x_coord + self.getWidth('_NCAPMet2RXRouting') / 2,#self._DesignParameter['RES']['_DesignObj']._DesignParameter['_Met1Layer']['_XYCoordinates'][1][1] + self.getWidth('_NCAPMet2RXRouting') / 2,
																					 self.getXY('_NCAPMet2RXRouting')[0][0][1]]],
																				   [[self._DesignParameter['RES']['_XYCoordinates'][0][0] + res_x_coord, #self._DesignParameter['RES']['_DesignObj']._DesignParameter['_Met1Layer']['_XYCoordinates'][1][1],
																					 self.getXY('_NCAPMet2RXRouting')[0][0][1]],
																					[self._DesignParameter['RES']['_XYCoordinates'][0][0] + res_x_coord, #self._DesignParameter['RES']['_DesignObj']._DesignParameter['_Met1Layer']['_XYCoordinates'][1][1],
		...
		return (N0Dummy_Finger, N1Dummy_Finger, P0Dummy_Finger, P1Dummy_Finger, P2Dummy_Finger)
# execution time
...
# 	from generatorLib.generator_models import DRCchecker_test2 as DRCchecker
# 	from Private import Myinfo
# 	from Netlist import amp_src
# 	import json
# 	import os
#
# 	# print(f"{i}nd loop")
...
# 	# while i < 15000:
# 	# 	try:
# 	# 		Insert_comp = True  #random.choice([True, False])
# 	# 		P0_Finger = DRCchecker.RandomParam(2, 20, 1)    #random.randint(2,30)
# 	# 		P1_Finger = P0_Finger
# 	# 		P2_Finger = DRCchecker.RandomParam(2, 20, 1)    #random.randint(2,30)
# 	# 		P3_Finger = DRCchecker.RandomParam(2, 20, 1)    #random.randint(2,30)
# 	#
# 	# 		N0_Finger = DRCchecker.RandomParam(2, 20, 1)    #random.randint(2,30)
# 	# 		N1_Finger = N0_Finger
# 	# 		N2_Finger = DRCchecker.RandomParam(2, 20, 1)    #random.randint(2,30)
# 	# 		N3_Finger = DRCchecker.RandomParam(2, 20, 1)    #random.randint(2,30)
# 	# 		N4_Finger = DRCchecker.RandomParam(2, 20, 1)    #random.randint(2,30)
# 	#
# 	# 		ChannelLength = DRCchecker.RandomParam(50, 200, 2)    #random.randrange(50,200,2)
# 	# 		MOS_Width = DRCchecker.RandomParam(500, 1200, 2)    #random.randrange(500,1200,2)
# 	# 		GateSpacing = 114
# 	# 		SDWidth = 60
# 	# 		XVT = 'SLVT'    #random.choice(['HVT','RVT','LVT','SLVT'])
# 	#
# 	# 		NCAP_XWidth = DRCchecker.RandomParam(1000, 3000, 10)  #random.randrange(1000,5000,2)
# 	# 		NCAP_YWidth = DRCchecker.RandomParam(1000, 3000, 10)  #random.randrange(1000,5000,2)
# 	# 		NCAP_NumofGate = DRCchecker.RandomParam(1, 3, 1)#1   #random.randint(1,4)
# 	# 		NCAP_NumofRX = DRCchecker.RandomParam(1, 3, 1)#1    #random.randint(1,4)
# 	#
# 	# 		ResWidth = DRCchecker.RandomParam(1000, 2000, 10) #random.randrange(1000,3000,2)
# 	# 		ResLength = DRCchecker.RandomParam(1000, 2000, 10)    #random.randrange(1000,3000,2)
# 	# 		ResCONUMX = None
# 	# 		ResCONUMY = 1
# 	# 		_SeriesStripes = DRCchecker.RandomParam(2, 4, 1)#2  #random.randint(1,4)
# 	# 		_ParallelStripes = 1
# 	#
# 	# 		_GuardringCOpitch = 170#random.randrange(150, 200, 2)
# 	# 		_GuardringbyArea = True
# 	# 		_GuardringNumofCO = None
# 	# 		_GuardringThickness = 350 #random.randrange(300, 500, 2)
# 	# 		_GuardringPPEnclosure = 56 #random.randrange(56, 100, 2)
#
# 	# 		par = (
# 	# 			P0_Finger, P1_Finger, P2_Finger, P3_Finger,
# 	# 			N0_Finger, N1_Finger, N2_Finger, N3_Finger, N4_Finger,
# 	# 			GateSpacing, SDWidth,
# 	# 			ChannelLength, MOS_Width,
# 	# 			NCAP_XWidth, NCAP_YWidth, ResWidth, ResLength
# 	# 		)
# 	# 		if par in par_list:
# 	# 			continue
# 	# 		TopObj = _SingleEndedOPAmp(_DesignParameter=None, _Name='OPAmp')
# 	# 		dummies = TopObj._CalculateDesignParameter(
...
# 	# 			'_GuardringThickness': _GuardringThickness,
# 	# 			'_GuardringPPEnclosure': _GuardringPPEnclosure,
# 	# 			'N0Dummy_Finger': dummies[0],
# 	# 			'N1Dummy_Finger': dummies[1],
# 	# 			'P0Dummy_Finger': dummies[2],
# 	# 			'P1Dummy_Finger': dummies[3],
# 	# 			'P2Dummy_Finger': dummies[4]
# 	# 		}
# 	# 		all_params[i] = Inputparams
# 	# 		# directory = f'{My.save_dir}/OPAmp/'
# 	# 		# if not os.path.exists(directory):
# 	# 		# 	os.makedirs(directory)
# 	# 		with open(f'{My.save_dir}/OPAmp_new/{cellname}_{i}.gds', 'wb') as testStreamFile:
# 	# 		# with open(f'./{cellname}_{i}.gds', 'wb') as testStreamFile:
# 	# 			tmp = TopObj._CreateGDSStream(TopObj._DesignParameter['_GDSFile']['_GDSFile'])
# 	# 			tmp.write_binary_gds_stream(testStreamFile)
# 	# 			testStreamFile.close()
# 	#
# 	# 		Schematic = amp_src.amp_gen(Inputparams)
# 	# 		with open(f'{My.save_dir}/OPAmp_new/{cellname}_{i}.src.net', 'w') as f:
# 	# 		# with open(f'./{cellname}_{i}.src.net', 'w') as f:
# 	# 			f.write(Schematic)
# 	# 			f.close()
# 	#
# 	# 		i += 1
...
# 	# 		print(f"Error in {i}th loop")
# 	# 		continue
# 	#
# 	# with open(f'{My.save_dir}/OPAmp_new/all_params.json', 'w') as f:
...
if __name__ == '__main__':
    ...
    LayoutObj =LDO_OPAmp(_Name=cellname)
