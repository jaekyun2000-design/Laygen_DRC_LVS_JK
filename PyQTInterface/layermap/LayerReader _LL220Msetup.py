...
def run_for_process_update():
    ...
    if _Technology == 'SS28nm':
        ...
        _LayDatNameTmp = _ExtendLayDatNum2LayDatName(_ExtendLayerMappingTmp)
        _LayerNum2CommonName = _LayerNumber2CommonLayerName(_LayerMappingTmp)
    elif _Technology == 'LL220M':
        _LayerMapFile = open(_HomeDirectory + '/PyQTInterface/layermap/LL220M/ll220m_tech.layermap')
        _LayerMappingTmp, _ExtendLayerMappingTmp = _ReadLayerMapFile(_LayerMapFile, 'VIRTUOSO')
        _LayerNameTmp = _LayerNumber2LayerName(_LayerMappingTmp)
        # _LayDatNameTmp = _LayDatNum2LayDatName(_LayerMappingTmp)
        _LayDatNameTmp = _ExtendLayDatNum2LayDatName(_ExtendLayerMappingTmp)
    ...
    ###added by minseo for ll220m -start

    if _Technology == 'LL220M':
        _LayerMapping.update({'LVT': _LayerMappingTmp[('LVT', 'drawing')]})
    ...
    if _Technology == 'LL220M':
        ...
        if _Technology == 'LL220M':
            _LayerMapping.update({'ALPHAddi': _LayerMappingTmp[('ALPHA', 'ddi')]})
            _LayerMapping.update({'BGddi': _LayerMappingTmp[('BG', 'ddi')]})
            _LayerMapping.update({'BNddi': _LayerMappingTmp[('BN', 'ddi')]})
            _LayerMapping.update({'DTddi': _LayerMappingTmp[('DT', 'ddi')]})
            _LayerMapping.update({'HVNWELLddi': _LayerMappingTmp[('HVNWELL', 'ddi')]})
            _LayerMapping.update({'MCCAPddi': _LayerMappingTmp[('MCCAP', 'ddi')]})
            _LayerMapping.update({'N3ddi': _LayerMappingTmp[('N3', 'ddi')]})
            _LayerMapping.update({'VBARddi': _LayerMappingTmp[('VBAR', 'ddi')]})
            _LayerMapping.update({'XRddi': _LayerMappingTmp[('XR', 'ddi')]})
            _LayerMapping.update({'ZGddi': _LayerMappingTmp[('ZG', 'ddi')]})

        ###added by minseo for ll220m -end
    _LayerMapFile.close()
