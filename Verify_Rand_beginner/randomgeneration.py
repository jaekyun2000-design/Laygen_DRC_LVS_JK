...
class RandGen:
    ...
    def run(self, iteration=2):
        ...
        for i in tqdm(range(1,step+1), file=sys.stdout):
            ...
            # For 1st run
            # commands = [(f"cd {personal.RUNDIR}/{word}; cp {personal.TECHDIR}/{self.cell_name}_oceanScript.ocn {self.cell_name}_oceanScript.ocn; "
            #              f"sed -i '2s,.*,resultsDir( \"/mnt/sdb/jihoon0721/simulation/Test_{self.cell_name}/spectre/schematic\"),' oceanScript.ocn") for word in range(1, worker+1)]
            # for cmd in commands:
            #     self.execute_command(cmd)
...
if __name__=="__main__":
    ...
    drc_result, lvs_result, pex_result, posim_result = randgen.run(20)
